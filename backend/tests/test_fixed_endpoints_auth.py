#!/usr/bin/env python3
"""
Test suite for fixed API endpoints with authentication
"""
...
def login():
    ...
    try:
        response = session.get(f"{BASE_URL}/api/auth/status", timeout=10)
        print(f"Auth status check: {response.status_code}")
        if response.status_code == 200:
            data = response.json()
            print(f"Current auth status: {json.dumps(data, indent=2)}")
    ...
    # Try manual registration (creates user if doesn't exist)
    try:
        ...
        response = session.post(
            f"{BASE_URL}/api/auth/manual/register",
            json=reg_data,
            timeout=10
        )
    ...
    try:
        ...
        response = session.post(
            f"{BASE_URL}/api/auth/manual/login",
            json=login_data,
            timeout=10
        )
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"Login successful: {json.dumps(data, indent=2)}")
...
def main():
    print(f"\n{'#'*80}")
    print("TESTING FIXED API ENDPOINTS (WITH AUTHENTICATION)")
